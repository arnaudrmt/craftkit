...
import fr.arnaud.craftkit.api.*;
import net.md_5.bungee.api.chat.ClickEvent;
import org.bukkit.Material;
import org.bukkit.entity.Player;

/**
 * ExampleUsage demonstrates how to use the CraftKit API.
...
public class ExampleUsage {
    ...
    public void openNamePrompt(Player player) {
        new AnvilPrompt(player, "Type name", (p, text) -> {
    ...
    public void openSearchBar(Player player) {
        new SignPrompt(player, (p, lines, combinedLines) ->
    ...
    public void enterPassword(Player player) {
        new ChatPrompt(player, "§aYou may enter your password:", (p, message) ->
                p.sendMessage("§aLogging in..."));
    }

    /**
     * Adds example hub items to the player's inventory.
     * Demonstrates how to create clickable and custom items using ItemBuilder.
     *
    ...
    public void setHubItem(Player player) {
        ...
        ItemBuilder serverItem = new ItemBuilder(Material.COMPASS, "§6Servers");
        serverItem.setLore("", "§7Right-click to open the server menu");

        // Create a "Profile" skull item personalized to the player
        ItemBuilder profileItem = new ItemBuilder(Material.SKULL_ITEM, "§6Profile");
    ...
    /**
     * Sends an interactive, multi-line message to the player using MessageBuilder.
